...
async function addPoint(req, res) {
  try {
    ...
    const { profileId } = req.user; // Business profile ID
    ...
    // Ajouter le(s) point(s) via le service de fidélité
...
async function getCustomers(req, res) {
  try {
    ...
    // Obtenir les IDs uniques des clients
    ...
    res.json({
      ...
      pagination: {
        ...
        limit: parseInt(limit),
        offset: parseInt(offset),
        hasMore: (parseInt(offset) + (customers?.length || 0)) < (count || 0)
      }
    });
  } catch (error) {
